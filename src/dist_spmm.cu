...
int main(int argc, char *argv[]) {
    ...
    if (argc < 7)   {
        std::cerr << "Usage: ./dist_spmm <file_name> <dims> <task_dist> <comm_type> <n_rolls> <repeat> " << std::endl;
        return EXIT_FAILURE;
    }
    ...
    int nvlink_num_producers = 2; 
    int ib_num_producers = 2;

    // if (argc == 8) {
    //     nvlink_num_producers = std::stoi(argv[7]);
    // }
    // if (argc == 9) {
    //     ib_num_producers = std::stoi(argv[8]);
    // }
    ...
    for (int time_it = 0; time_it < repeat; ++time_it) {
        ...
        int ready_out_buff_id = 0;


        for (int node_pp = 0; node_pp < num_nodes; ++node_pp) {
            
            for (int local_tile_id = 0; local_tile_id < n_rolls; ++local_tile_id) {
                
                int node_id = (node_pp + my_node_id) % num_nodes;

                if (node_id != my_node_id) {
                    ready_out_buff_id = comm_manager.outer_buff_front();
                    CUDA_CHECK( cudaStreamWaitEvent(nvlink_stream,  ib_require_events[ready_out_buff_id], 0));
                    nvshmemx_team_sync_on_stream(NVSHMEMX_TEAM_NODE, nvlink_stream);  // ensure all GPUs on the node have finished prefetching
                    comm_manager.set_outer_buff_id();
                }

                if ( pre_tile_id < n_rolls && pre_node_pp < num_nodes) {
                    if (node_pp == 0 && pre_tile_id > ib_num_producers - 1) continue;
                    int last_out_buff_id = comm_manager.outer_buff_back();
                    int out_buff_id = (last_out_buff_id + 1) % ib_num_producers;
                    int remote_node_id = (pre_node_pp + my_node_id) % num_nodes;
                    // if (mype ==0) {
                    //     std::cout << "prefetch tile: " << pre_tile_id << " from node: " << remote_node_id 
                    //             << " to out_buff_id: " << out_buff_id << " queue head buff id : " << comm_manager.outer_buff_front() << std::endl << std::flush;

                    // }
                    if (out_buff_id == comm_manager.outer_buff_front()) {
                        CUDA_CHECK( cudaStreamWaitEvent(ib_stream, ib_release_events[out_buff_id], 0));
                    }
                    comm_manager.comm_on_stream_ib(out_buff_id, pre_tile_id, remote_node_id, ib_stream);
                    CUDA_CHECK( cudaEventRecord( ib_require_events[out_buff_id], ib_stream));
                    pre_tile_id += 1;
                    if (pre_tile_id == n_rolls) {
                        pre_tile_id = 0;
                        pre_node_pp += 1; // reset to 1, because 0 is my node
                    }
                }
                ...
                if (node_id != my_node_id) {
                    CUDA_CHECK( cudaEventRecord(ib_release_events[ready_out_buff_id], compute_stream));
                }
